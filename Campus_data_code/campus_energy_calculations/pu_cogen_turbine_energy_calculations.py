''' Function to calculate kiloWatts per hour from Princeton Cogen Turbines: EP.Cogen.Turbine.Power.ION.Total_kW. Adds the data to InfluxDB. '''
import pandas as pd
...
# Build query for database query of last 2 points for power from Princeton Cogen Turbines; Icetec parameter: "EP.Cogen.Turbine.Power.ION.Total_kW"
def influxdb_query_builder_cogen_turbine_kW() :
    # Set data_field provided to function to variable "data_field_label_str"
    data_field_label_str = str("EP.Cogen.Turbine.Power.ION.Total_kW")
...
def query_last_2_values_cogen_turbine_InfluxDB() :
    # Generate Queries for Cogen Turbine data; only last 2 points will be used
    cogen_turbine_query = influxdb_query_builder_cogen_turbine_kW()
    # print(cogen_turbine_query)   # Debugging print statement

    query_str = str(cogen_turbine_query)  # convert query to type string
...
# Debugging function
# print(query_last_2_values_cogen_turbine_InfluxDB()) # Debugging print statement


# Function to calculate kiloWatts per hour from Princeton Cogen Turbine; Icetec parameter: "EP.Cogen.Turbine.Power.ION.Total_kW"
def write_cogen_turbine_energy_to_InfluxDB() :
    dict_of_data_dt_kW = { }  # Initialize dictionary

    # Returns a dictionary of timestamp and kW values from "EP.Cogen.Turbine.Power.ION.Total_kW"
    dict_of_data_dt_kW = query_last_2_values_cogen_turbine_InfluxDB()
    ...
    average_power_cogen_turbine_kW = float((last_value_kW + first_value_kW) / 2)
    average_power_cogen_turbine_kW = round(float(average_power_cogen_turbine_kW) , 4)
    ...
    kW_per_hour = average_power_cogen_turbine_kW * time_difference_hr
    # print(kW_per_hour)

    # Puts data in json body for writing to InfluxDB
    json_body = [
        {
            'measurement' : "PU Energy Data: Cogen Turbine" ,
            'tags' : {
    ...
    # print("PU Energy Data: Cogen Turbine")  # Debugging print statement
    # print(json_body) # Debugging print statement


# Debugging function
# print(write_cogen_turbine_energy_to_InfluxDB())  # Debugging print statement
